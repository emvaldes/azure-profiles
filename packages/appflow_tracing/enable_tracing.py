...
def get_log_file(caller_log_path=None):
    """Determine the correct log file path based on the caller module's request or self-inspection."""
    appflow_logsdir = Path(CONFIG["logging"]["package_logsdirname"])
    appflow_logsdir.mkdir(parents=True, exist_ok=True)
    ...
    if caller_log_path:
        ...
        return appflow_logsdir / f"{CONFIG['logging']['log_filename']}_{timestamp}.log"
...
def log_message(message, category="", json_data=None, serialize_json=False):
    ...
    if CONFIG["logging"]["enable_tracing"]:
        color = CONFIG["colors"].get(category, CONFIG["colors"]["RESET"])
        print(f"{color}{message}{CONFIG['colors']['RESET']}")
        if json_data:
        ...
        #
        # print( f'config.logging.enable_tracing: {CONFIG["logging"]["enable_tracing"]}' )
        # print( f'config.logging.enable_logging: {CONFIG["logging"]["enable_logging"]}' )

def manage_logfiles():
    """Remove the oldest log files if they exceed the max_logfiles limit, ensuring logs are managed per package."""
    logs_basedir = Path(CONFIG["logging"]["logs_dirname"])
    for log_subdir in logs_basedir.iterdir():
        if log_subdir.is_dir():  # Ensure it's a directory
            ...
            num_to_remove = len(log_files) - (CONFIG["logging"]["max_logfiles"])
            if num_to_remove > 0:
...
def start_tracing():
    """Public function to enable tracing for external modules."""
...
def trace_all(frame, event, arg):
    ...
    #
    # log_message(f"🔍 Tracing Activated: {event} in {frame.f_code.co_name}", "DEBUG")

    if not CONFIG["logging"]["enable_logging"] and not CONFIG["logging"]["enable_tracing"]:
    ...
    if event == "call":
        ...
        if caller_frame:
            try:
                ...
                # log_message(f"\n[CALL] {caller_filename}[{caller_lineno}] ( {invoking_line} ) -> {relative_path(filename)} ({frame.f_code.co_name})[{frame.f_code.co_firstlineno}]", "CALL")
                log_message(f"\n[CALL] {caller_filename} ( {invoking_line} )", "CALL")
        ...
        try:
            ...
            # log_message(f"\n[RETURN] {return_filename}[{return_lineno}] -> RETURN VALUE (Type: {return_type}): {return_value}", "RETURN")
...
def main():
    ...
    log_message("Executing appflow_tracing in standalone mode.", "IMPORT")
